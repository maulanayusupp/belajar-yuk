...
| `pages/coding/`    | **Modul TERPISAH** (bukan subject): peta level (`index`) + main level (`[id]`). Puzzle grid ala Lightbot                                                                                  | `codingService` (progres sendiri, `belajar-yuk:coding:*`), `codeRunner` (step-tree, `countBlocks`, gem-collect win). Grid char: `.` path · `#` tembok · `G` tujuan · `C` permata (kumpulkan semua). Runner per `level.concept`: `CodingLevelRunner` (sequence/gems) / `CodingLoopRunner` (loop). Konten: `data/coding/<world>/index.ts` |
