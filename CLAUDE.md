...
| `pages/coding/`    | **Modul TERPISAH** (bukan subject): peta level (`index`) + main level (`[id]`). Puzzle grid ala Lightbot                                                                                  | `codingService` (progres sendiri, `belajar-yuk:coding:*`). **Tiap materi boleh punya BENTUK GAME sendiri** via `level.kind`: `grid` (puzzle robot — `codeRunner` step-tree + `countBlocks`, grid char `.`/`#`/`G`/`C`gem; runner `CodingLevelRunner`/`CodingLoopRunner` per `concept`) & `order` (susun kartu langkah — `CodingOrderRunner`, bukan grid). Dispatch di `pages/coding/[id].vue`. Tambah bentuk game baru = tipe level baru + runner baru. Konten: `data/coding/<world>/index.ts` |
