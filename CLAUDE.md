...
| `pages/coding/`    | **Modul TERPISAH** (bukan subject): peta level (`index`) + main level (`[id]`). Puzzle grid ala Lightbot                                                                                  | `codingService` (progres sendiri, `belajar-yuk:coding:*`), `codeRunner` (step-tree + `countBlocks`), `CodingPuzzleGrid`. Runner per konsep: `CodingLevelRunner` (sequence) / `CodingLoopRunner` (loop). Konten: `data/coding/<world>/index.ts` |
