...
| `pages/coding/`    | **Modul TERPISAH** (bukan subject): peta level (`index`) + main level (`[id]`). Puzzle grid ala Lightbot                                                                                  | `codingService` (progres sendiri, `belajar-yuk:coding:*`), `codeRunner`, `CodingPuzzleGrid`/`CodingLevelRunner`. Konten: `data/coding/<world>/index.ts` |
